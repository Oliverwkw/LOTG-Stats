...
- [x] **2021 rookie draft Original Team** (#262, corrected by #263) — Sleeper mislabeled the linear 2021 rookie draft as snake, so R2/R4 `Original Team` read off the reversed even-round draft_slot (repeats: LWebs53×2 etc.) and `2021 2.??` never resolved. Fix: take Original Team from the pick's NUMBER **position** in the round → stevenb123, Jacob, AceMatthew, BRO, plehv79, LWebs53, Oliverwkw, shmuel256 (8 distinct, no repeats). **#263 correction:** #262 also wrongly linearized the pick number/player (showed 2.01=Carter); the number+player are keyed by Sleeper's draft_slot and were already right (2.01 Justin Fields … 2.08 Michael Carter) — restored. Player↔Number + Final-Team-per-player unchanged from pre-#262. Commissioner moves re-determined from the chains. Closes #261's lone `2.??` cell. **#263 also (per user audit):** don't flag a self-drafted pick (4.03); 4 manual drafter corrections via `_DRAFTER_FIX_2021` (Elijah Moore + Nico Collins → plehv79, Dyami Brown → AceMatthew, Rhamondre Stevenson → shmuel256), each verified by tracing the rookie's first transaction; merged Sleeper's botched draft-day Carter/Stevenson trade (pick swap + phantom player swap) into one picks-only trade (2.08 ↔ 3.06 + 2022 4.07 + 2023 4.08, matched by Sleeper ids 7607/7611). **Result: all 32 2021 rookies trace start-to-finish from their true drafter; commish moves settle at 7.** Phase 13 ESPN backfill can refine these chains into explicit trade legs.
