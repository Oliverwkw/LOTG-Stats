...
- [x] **2021 rookie draft Original Team** (#262) — Sleeper mislabeled the linear 2021 rookie draft as snake, so R2/R4 `Original Team` repeated (LWebs53×2 etc.) and `2021 2.??` never resolved. Fix: linearize non-vet snake drafts (numbering + original-team-by-slot → stevenb123, Jacob, AceMatthew, BRO, plehv79, LWebs53, Oliverwkw, shmuel256, no repeats); repair fixes the corrupted **Final/drafter** only (Player→Final unchanged for all 32 picks). Commissioner moves re-determined from the rebuilt chains: 13 untracked startup hops flagged True, real ledger trades (2.08 J. Fields) stay False. Closes #261's lone `2.??` cell. Note: Phase 13 ESPN backfill can refine these chains into explicit trade legs.
