...
- [ ] **PR D — Awards + streaks:** awards = **One-man army** (team_week: highest single-starter share of team PF), **Most bench points** (team_week), **Captain** (player_week: biggest single-team carry); streaks as RUNNING team_week columns → team_year/all_time longest (mirror existing Win/Loss streak) = top-half, bottom-half, playoff-appearance (consecutive seasons), winning-season (consecutive seasons), **150+ PF**, standings-leader, quiet (no roster moves), and **rivalry** = longest H2H win streak vs EACH opponent (team_all_time per-opponent like "Record vs [team]" + a running team_week "Win streak vs this opponent" that updates on a win).
- [ ] **PR E — in-season freshness audit:** confirm the data-update timeline for each upstream source (nflverse weekly stats / weekly rosters / injuries, DynastyDaddy KTC daily history, Sleeper) and determine which columns need adjustment so they update CORRECTLY mid-season (forward-looking windows, "most-recent KTC", current-season tenures, anything anchored to "today" or end-of-season). Output: list of columns that go stale/incorrect in-season + the fix for each.
