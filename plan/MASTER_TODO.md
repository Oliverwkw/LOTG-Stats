...
- [x] **#6 Trades next/previous links** (#261) — every NON-FAAB per-asset next/previous cell now links. The xlsx explodes the per-asset link columns into one clickable column per received asset; at a chain endpoint there was no event ref so the cell showed the name unlinked (190 next + 319 prev non-FAAB). Fix: picks fall back to their picks-sheet home row (`pick_home_phref` → PH#, builder; pick prev=N/A 170→1); players link the name to their `player_all_time` row (xlsx writer, Phase 11D map). FAAB stays unlinked (documented exception). Only remaining unlinked non-FAAB cell = `2021 2.??` (undetermined pre-Sleeper slot, no home row → Phase 13). Verified: player-home 134/134 + pick-home 61/61 link to correct rows; CSV diff confined to trades prev column.
  - **NOTE (Phase 13 dependency):** a trade may currently be an asset's *start point* (`previous` = N/A on its first-ever trade) because pre-Sleeper provenance doesn't exist yet — that's acceptable **for now**. Once the **ESPN 2020 + off-platform pick-trade backfill** (Phase 13) lands, a trade must **never** be an asset's origin: every asset's first trade should chain back to an earlier draft/acquisition. Re-audit these `previous`=N/A endpoints after Phase 13 and require a working back-link.
