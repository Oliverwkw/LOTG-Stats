...
5. **All-time boards** (every row-level sheet) — `picks`, `trades`,
   `transactions`, and equally `player_year` / `team_year` / `league_year` and
   `player_week` / `team_week` / `league_week`. A sheet's rows are its records:
   a pick, a trade, a transaction, but also one player's season and one team's
   week. **Every numeric column of every such sheet is ranked over every row of
   that sheet ever**, the top/bottom 5 are snapshotted, and week over week the
   board is diffed and phrased exactly like a player / team all-time crossing:
   *"Oliverwkw's 2023-12-05 trade for DeAndre Hopkins, 2025 3.08(J. Milroe)
   +2 more passes shmuel256's 2020-09-15 trade for Justin Jefferson for
   1st-highest O-Score (103.3)."*

   Covering every season — not just the in-progress one — is what makes a
   recompute of HISTORY reportable. When a rolling KTC window or a formula change
   re-values a 2023 trade into the all-time O-Score top 5, the row that moved is a
   past-season row; a board holding only the current season's rows produced no
   item at all. Only the **mover** is reported: everyone it passed is pushed down
   a place and gets no line, the same riser-only convention `CROSSING_CONFIG` uses
   for the all-time team boards. A place is identified by the sheet's natural row
   key (not the human label, which repeats when a team picks the same player up
   twice; not the row index, which renumbers whenever a row is inserted). Event
   labels name the assets — what a trade received, who a transaction added —
   capped at three plus "+N more".

   **Nothing numeric is filtered.** The all-time crossings no longer skip the
   "build-volatile" families that #380 excluded (the `skill` family, `Luck` /
   `Avg yearly luck`, `Hardship`, `Tanking`, `Future draft capital`,
   `Startup draft players remaining`, `Number of teams`): a reorder is a reorder,
   and those are columns a reader wants to hear about. The weekly AUDIT still
   exempts them — there the question is "did a frozen historical value move",
   which is a different one, and `lib/lotg_support/volatile_columns.py` now has
   that single consumer.

   **Mirrored columns rank on their positive side only.** A matchup's margin is
   +M for the winner and −M for the loser; a trade's net KTC is +X / −X. Ranking
   the raw values would report one fact as two records and fill the bottom of the
   board with the mirror image of its top. So `Margin`, `Difference in pregame avg
   max PF from opponent` and the five trade differentials (`KTC value difference`
   × 4, `Asset difference in average age`) are ranked over positives: 1st-highest
   Margin is the biggest blowout, 1st-lowest is the closest game, each named once
   from the side that came out ahead. Detected **structurally**, from rows that
   pair through a counterpart column — not from the shape of the distribution,
   which would also catch `Change from previous week` (symmetric because players
   gain and lose about equally often) and throw away "biggest drop from last
   week", a real record.

   Volume, replayed against the real Tuesday vintages (runs 454→456→459→461):
   18, 15 and 0 board moves on top of 4, 4 and 0 all-time crossings — so a quiet
   week is still quiet and `--skip-empty` still suppresses it.
