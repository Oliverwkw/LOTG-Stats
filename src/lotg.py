...
def build_all(repo_root: Path) -> None:
    ...
    def _seed_pick_ledger(season_id: int, roster_ids: List[int], rounds: int) -> None:
        if season_id < 2021:
            return
        for rnd in range(1, int(rounds) + 1):
            for rid in roster_ids:
                key = (int(season_id), int(rnd), int(rid))
                if key in pick_current_owner:
                    continue
                pick_current_owner[key] = int(rid)
                pick_trade_events[key] = []
                pick_holdings[(int(season_id), int(rnd), int(rid))].append(int(rid))

    def _slot_for_roster(season_id: int, roster_id: int) -> Optional[int]:
        roster_ids = sorted(roster_ids_by_season.get(int(season_id), []))
        if not roster_ids:
            return None
        try:
    ...
    def _format_pick_number(
        ...
        if round_num is None:
            return None
        team_count = len(roster_ids_by_season.get(int(season_id), [])) or None
        if pick_no is not None and team_count:
            slot = ((int(pick_no) - 1) % int(team_count)) + 1
            return f"{int(round_num)}.{slot:02d}"
        slot = _slot_for_roster(int(season_id), int(original_owner)) if original_owner is not None else None
        if slot is None:
            return f"{int(round_num)}.??"
        return f"{int(round_num)}.{slot:02d}"

    def _format_pick_label(
        season_id: int,
        round_num: Optional[int],
        original_owner: Optional[int],
    ) -> Optional[str]:
        number = _format_pick_number(int(season_id), round_num, None, original_owner)
        if number is None:
            return None
        return f"{int(season_id)} {number}"
    ...
    def _resolve_pick_key(
        ...
        if original_owner is not None:
            key = (int(season_id), int(round_num), int(original_owner))
            if pick_current_owner.get(key) == int(prev_owner):
                return key
        holdings = sorted(pick_holdings.get((int(season_id), int(round_num), int(prev_owner)), []))
        if len(holdings) == 1:
            return (int(season_id), int(round_num), int(holdings[0]))
        if holdings:
            return (int(season_id), int(round_num), int(holdings[0]))
        return None
    ...
    for lg in leagues:
        ...
        if draft_rounds_by_season[season] and roster_ids_by_season[season]:
            _seed_pick_ledger(int(season), roster_ids_by_season[season], int(draft_rounds_by_season[season]))
...
# draft picks history (rookie + startup as available in Sleeper; still partial)
            ...
            for t in tx_by_week.get(wk, []):
                try:
                    ...
                    if ttype == "trade":
                        ...
                        if draft_picks:
                            dp_sorted = sorted(
                                [dp for dp in draft_picks if isinstance(dp, dict)],
                                key=lambda dp: (
                                    _to_int(dp.get("season"), season) or 0,
                                    _to_int(dp.get("round"), 0) or 0,
                                    _to_int(dp.get("previous_owner_id") or dp.get("previous_owner"), 0) or 0,
                                    _to_int(dp.get("owner_id") or dp.get("roster_id"), 0) or 0,
                                    _to_int(dp.get("original_owner_id") or dp.get("original_owner"), 0) or 0,
                                ),
                            )
                            for dp in dp_sorted:
                                ...
                                if dp_season is None or dp_round is None or prev_owner is None or new_owner is None:
                                    continue
                                if dp_season not in roster_ids_by_season:
                                    seed_rosters = roster_ids_by_season.get(season, [])
                                    seed_rounds = draft_rounds_by_season.get(season, 0)
                                    if seed_rosters and seed_rounds:
                                        _seed_pick_ledger(int(dp_season), seed_rosters, int(seed_rounds))
                                key = _resolve_pick_key(int(dp_season), int(dp_round), int(prev_owner), original_owner)
                                if not key:
                                    _log(
                                        debug,
                                ...
                                pick_holdings[(key[0], key[1], int(new_owner))] = sorted(
                                    set(pick_holdings.get((key[0], key[1], int(new_owner)), []) + [int(key[2])])
                                )
                                pick_current_owner[key] = int(new_owner)
                                pick_trade_events.setdefault(key, []).append((created_dt, int(prev_owner), int(new_owner), int(wk)))
                        ...
                        for dp in draft_picks:
                            ...
                            dp_round = _to_int(dp.get("round"), None)
                            original_owner = _to_int(dp.get("original_owner_id") or dp.get("original_owner"), None)
                            label = _format_pick_label(int(dp_season), dp_round, original_owner)
                            if label:
    ...
    if base_season is not None:
        ...
        seed_source = int(latest_league_season or base_season)
        seed_rosters = roster_ids_by_season.get(seed_source, [])
        seed_rounds = draft_rounds_by_season.get(seed_source, 0)
        if seed_rosters and seed_rounds:
            for yr in range(2021, max_future_season + 1):
                if yr not in roster_ids_by_season:
                    roster_ids_by_season[yr] = seed_rosters
                if yr not in draft_rounds_by_season:
                    draft_rounds_by_season[yr] = seed_rounds
                _seed_pick_ledger(int(yr), roster_ids_by_season[yr], int(draft_rounds_by_season[yr]))
    ...
    # --------------------------
    # Build draft pick history from transaction ledger + draft results.
    # --------------------------
    try:
        pick_rows = []
        if pick_current_owner:
            ...
            for season_id, mapping in roster_to_team_by_season.items():
                for rid, name in mapping.items():
                    roster_name_fallback.setdefault(int(rid), str(name))

            def _roster_name(season_id: int, rid: Optional[int]) -> Optional[str]:
                if rid is None:
                    return None
                name_map = roster_to_team_by_season.get(int(season_id), {})
            ...
            for rec in draft_picks_records:
                if rec.get("draft_season") is None or rec.get("round") is None or rec.get("roster_id") is None:
                    continue
                picks_by_bucket[(int(rec["draft_season"]), int(rec["round"]), int(rec["roster_id"]))].append(rec)

            assigned_pick_by_key: Dict[Tuple[int, int, int], Dict[str, Any]] = {}
            for (p_season, p_round, owner_id), picks in picks_by_bucket.items():
                keys = [
                    k for k, owner in pick_current_owner.items()
                    if k[0] == p_season and k[1] == p_round and owner == owner_id
                ]
                keys_sorted = sorted(keys, key=lambda k: (_slot_for_roster(p_season, k[2]) or 0, k[2]))
                picks_sorted = sorted(picks, key=lambda r: (r.get("pick_no") or 0))
                for rec, key in zip(picks_sorted, keys_sorted):
                    assigned_pick_by_key[key] = rec
                if len(picks_sorted) != len(keys_sorted):
                    _log(
                        debug,
                        f"[{_now_iso()}] WARN pick ledger mismatch: season={p_season} round={p_round} owner={owner_id} picks={len(picks_sorted)} keys={len(keys_sorted)}",
                    )

            def _chain_for_key(key: Tuple[int, int, int]) -> List[int]:
            ...
            for key, current_owner in sorted(pick_current_owner.items()):
                season_id, round_num, original_owner = key
                if season_id < 2021:
                    continue
                draft_rec = assigned_pick_by_key.get(key)
                pick_no = draft_rec.get("pick_no") if draft_rec else None
                roster_id = draft_rec.get("roster_id") if draft_rec else current_owner
                player = draft_rec.get("player_id") if draft_rec else None
                pick_made_by = _roster_name(season_id, roster_id)
                chain = _chain_for_key(key)
                if roster_id is not None and (not chain or chain[-1] != int(roster_id)):
                    chain = chain + [int(roster_id)]
                pick_rows.append({
                    "Year": season_id,
                    "Pick made by": pick_made_by,
                    "Original Team": _roster_name(season_id, original_owner),
                    "Number": _format_pick_number(season_id, round_num, pick_no, original_owner),
                ...
                for j in range(1, min(10, len(chain))):
                    pick_rows[-1][f"Trade {j}"] = _roster_name(season_id, chain[j])
