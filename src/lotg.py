...
def build_all(repo_root: Path) -> None:
    ...
    # Draft pick ownership ledger (season, round, original_owner) -> current owner
    pick_current_owner: Dict[Tuple[int, int, int], int] = {}
    pick_trade_events: Dict[Tuple[int, int, int], List[Tuple[Optional[datetime], Optional[int], int, int]]] = {}
    ...
    def _format_pick_number_from_owner(season: int, round_num: Optional[int], owner_id: Optional[int]) -> Optional[str]:
        if round_num is None:
            return None
        if owner_id is None:
            return f"{int(round_num)}.??"
        slot = _slot_for_roster(season, int(owner_id))
        if slot is None:
            return f"{int(round_num)}.??"
        return f"{int(round_num)}.{slot:02d}"

    def _format_pick_label(season: int, round_num: Optional[int], owner_id: Optional[int]) -> Optional[str]:
        num = _format_pick_number_from_owner(season, round_num, owner_id)
        if num is None:
            return None
    ...
    def _select_pick_key(
        ...
        if original_owner is not None:
            ...
            if key in pick_current_owner and pick_current_owner[key] == int(prev_owner):
                return key
        candidates = pick_holdings.get((int(season), int(round_num), int(prev_owner)), [])
        if original_owner is not None and int(original_owner) in candidates:
            return (int(season), int(round_num), int(original_owner))
        if len(candidates) == 1:
...
# draft picks history (rookie + startup as available in Sleeper; still partial)
                ...
                try:
                    ...
                    if ttype == "trade":
                        ...
                        if draft_picks:
                            ...
                            for dp in dp_sorted:
                                ...
                                pick_trade_events.setdefault(key, []).append(
                                    (created_dt, wk, int(prev_owner), int(new_owner)),
                                )
                        ...
                        for dp in draft_picks:
                            ...
                            dp_round = _to_int(dp.get("round"), None)
                            orig_id = _to_int(
                                dp.get("original_owner_id") or dp.get("original_owner") or dp.get("previous_owner_id") or dp.get("previous_owner"),
                                None,
                            )
                            label = _format_pick_label(int(dp_season), dp_round, orig_id)
                            if label:
    ...
    # Ensure pick ledger includes seasons 2021 through three years after latest draft.
    latest_draft_season = max(
        [rec.get("draft_season") for rec in draft_picks_records if rec.get("draft_season") is not None],
        default=None,
    )
    ...
    if base_season is not None:
        ...
        seed_source = latest_league_season or base_season
    ...
    try:
        if pick_current_owner:
            ...
            def _trade_chain_for_key(key: Tuple[int, int, int]) -> List[int]:
                ...
                chain = [key[2]]
                for _, _, _, new_owner in events_sorted:
            ...
            for rec in draft_picks_records:
                ...
                chain = _trade_chain_for_key(key) if key else []
            ...
            # Fill missing/future picks from ledger (including 2021+ and +3 years).
            ...
            for key, owner_id in sorted(pick_current_owner.items()):
                ...
                number = _format_pick_number_from_owner(int(season_id), int(round_num), int(original_owner))
