...
def build_all(repo_root: Path) -> None:
    ...
    pick_trade_chain: Dict[Tuple[int, int, int], List[int]] = {}
    ...
    def _ensure_pick_bases(target_season: int, source_season: int) -> None:
        ...
        if target_season in draft_rounds_by_season:
            ...
            rounds = draft_rounds_by_season.get(source_season)
        if rounds is None:
            return
    ...
    # ------------- Build each season -------------
    for lg in sorted(leagues, key=lambda x: _to_int(x.get("season"), 0) or 0):
                    ...
                    if ttype == "trade":
                        ...
                        if draft_picks:
                            ...
                            for dp in dp_sorted:
                                ...
                                chain = pick_trade_chain.get(key, [key[2]])
                                if chain[-1] != int(new_owner):
                                    chain.append(int(new_owner))
                                pick_trade_chain[key] = chain
