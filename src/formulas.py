...
_ROWS = [
    ...
    },
    {
        ...
        "Sheet": "transactions",
        "Formula": "Mean fantasy points across the player's 5 most-recent PLAYED NFL games BEFORE the pickup date. Uses nflverse fantasy_points_ppr; falls back to our roster history (pw) when nflverse has no record. If fewer than 5 played games exist, averages whatever is available (e.g., 2 games → mean of 2).",
        "Notes": "'On team' is historical naming — the metric is really 'player's recent NFL form'. Includes weeks the player was unrostered. Bye weeks are not counted as played; injury-week treatment depends on whether nflverse logged any stats line.",
    },
    {
        "Stat": "Average PPG of dropped player over same time",
        "Sheet": "transactions",
        "Formula": "Same definition as 'Average PPG on team' but for Player Dropped.",
        "Notes": "Blank for add-only rows (no player dropped).",
    },
    {
        "Stat": "Difference of averages",
        "Sheet": "transactions",
        "Formula": "Average PPG on team − Average PPG of dropped player over same time.",
        "Notes": "Treats a missing side as 0; this can over- or under-state the diff if only one side resolves.",
    },
    {
        "Stat": "Difference of averages adjusted by position",
        "Sheet": "transactions",
        "Formula": "added_adj − dropped_adj, where adj = PPG × all_starter_avg / pos_avg[player_position]. all_starter_avg and pos_avg are league-wide all-time starter averages from player_week.",
        "Notes": "Normalises QB/RB/WR/TE scoring scales so a 12-PPG TE doesn't read the same as a 12-PPG QB. Falls back to unadjusted PPG when position isn't found.",
    },
    {
        "Stat": "Age difference",
        "Sheet": "transactions",
