...
_ROWS = [
    ...
    },
    {
        ...
        "Sheet": "trades",
        "Formula": "A composite, with WIN IMPACT heavily weighted, of five standardised (z-scored across all trades) player/asset signals (no KTC in the output), summed and scaled (×1500) into a KTC-like band. (1) WIN IMPACT, weight 2.0 — games the trade flipped PLUS a share of games flipped by later trades that re-used the received assets. Direct: for each post-trade week a received asset started, net points = received starters − the top-k players traded away; if (actual weekly margin) vs (margin − net) crosses 0, ±1 game. Downstream: when a received asset is later re-traded, this trade is credited that later trade's win impact × the FRACTION of the later trade's sent-side KTC value (on the later trade's day) that came from this trade's assets — recursively, so a minor add-on bundled later for a stud earns only its small value share. (2) realized production, Avg net points, 0.8; (3) trade value incl. picks, Trade addition value, 0.5; (4) future capital, Pick value received, 0.5; (5) youth, −Asset difference in average age, 0.3. All signals reward a rebuild, so tank trades aren't punished. Continuous → percentile-ranks cleanly.",
        "Notes": "Item 3 (v4). Earlier: v1 (win%×Tanking) degenerate; v2 (games-flipped alone) coarse, ~0.13 vs KTC. Win impact uses nflverse points + team_week margins; KTC is used ONLY to proportion the downstream credit, not in the output. Weights/scale tunable; per user, games-flipped is the heavily-weighted core. Correlation with 'KTC value difference at end of season' to be measured on the build (soft ~40% target).",
    },
    {
