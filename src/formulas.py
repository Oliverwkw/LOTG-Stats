...
_ROWS = [
    ...
    },
    {
        ...
        "Sheet": "trades",
        "Formula": "How many GAMES the trade actually flipped for the team, from player production. For each week after the trade in which a received asset STARTED, the week's net points = (received starters' points) − (the top-k players traded away that week, same maximize rule as Points lost). The counterfactual margin had the team NOT traded = (actual margin that week) − (net points). If the win/loss outcome differs between the real margin and the counterfactual, the trade is credited +1 (won a game the team would have lost) or −1 (lost a game it would have won). Σ over all such weeks × COEFF (1000 KTC-points per net game flipped, tunable). Picks count via the players drafted with them (their future weeks), so a rebuild is credited when its rookies win games and is NOT punished for shedding veterans (those lost weeks aren't counted until a received asset plays).",
        "Notes": "Item 3 (v2 — the earlier win%×Tanking version was degenerate for same-offseason trades and uncorrelated). Player points from the nflverse game log; weekly margins from team_week. N/A until a received asset has actually started a game for the team (future / not-yet-drafted-pick trades are blank, not negative). Calibrated to roughly track 'KTC value difference at end of season' (soft target).",
    },
    {
