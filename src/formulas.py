...
_ROWS = [
    ...
    },
    {
        ...
        "Formula": "An overall 0–100 score for the move: take FOUR stats, convert each to its PERCENTILE (0–100) across that sheet's rows, and average the four. The four (per sheet): picks → Avg points added, Player addition value, most-recent-KTC, Pick-adjusted Difference in Avg career PPG adjusted by position; transactions → Avg net points, Player addition value, most-recent-KTC, % of starts made while rostered; trades → Avg net points, Trade addition value, most-recent-KTC, Trade impact score. 'most-recent-KTC' = the percentile of the latest populated KTC value/difference on the row (scanning 5yr→2yr→1yr→end-of-rookie/season→deal/draft day). Percentiles use average tie-handling EXCEPT a value of exactly 0 (no production — e.g. the ~87% of adds that never started) is pushed to the bottom of its tie, so zeros cluster low rather than in the middle. N/A unless ALL FOUR components are present.",
        "Notes": "Item 4. ~50 means a middle-of-the-pack move on these four axes; ~80+ is a strong move. For picks, the 2021 vet/startup draft is excluded entirely — from every percentile pool AND its own rows are N/A. Percentiles are within each sheet (picks ranked vs picks, etc.).",
    },
    {
