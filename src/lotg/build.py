...
def build_all(repo_root: Path) -> None:
    ...
    if not pw.empty:
        ...
        # Normalize sorting by Player/Year and week number before computing deltas and heuristics.
        # We preserve original row index for later flag updates.
        pw = pw.sort_values(["Player", "Year", "Week"]).reset_index(drop=True)

        # -----------------------------------------------------------------
        # Recompute Injury? and Suspension? flags from scratch.  This logic
        # combines a conservative heuristic approach to infer missed
        # games from fantasy scoring patterns.  Because external data
        # sources such as nflverse injuries or snap counts may be
        # unavailable in this environment, we use only the per‑player
        # scoring history along with bye weeks and bench status.
        #
        # 1) Reset existing flags so we don’t inherit previous mislabels.
        pw["Injury?"] = False
        pw["Suspension?"] = False

        # 2) Fill missing NFL team values by forward/back filling within
        #    each player‑season.  This mitigates blanks that can
        #    propagate through downstream logic and makes NFL team
        #    assignment more stable historically.
        if "NFL team" in pw.columns:
            try:
                ...
                # Fallback: simple fill on entire column if groupby fails
                pw["NFL team"] = pw["NFL team"].ffill().bfill()

        try:
            # Convert Week values to numeric index (or sentinel 99) for sorting
            ...
            # Group by player and season and compute flags
            for (player, yr), g in pw.groupby(["Player", "Year"]):
                g = g.copy()
                # numeric week sort key
                g["_week_num"] = g["Week"].apply(_to_week_num)
                g = g.sort_values("_week_num")
                ...
                # index of first positive scoring numeric week
                first_pos_idx: Optional[int] = None
                for j, (p_val, w_num) in enumerate(zip(pts, week_nums)):
                    if w_num == 99:
                        continue
                    if p_val > 0:
                        first_pos_idx = j
                ...
                # Helper to assign flags for a contiguous run
                def assign_run(start_idx: int, end_idx: int) -> None:
                    length = end_idx - start_idx + 1
                    # Skip single‑week runs
                    if length < 2:
                        return
                    run_stats = statuses[start_idx:end_idx+1]
                    # If any week in run had a starter in fantasy lineup, don’t label
                    if any(s.lower() == "starter" for s in run_stats):
                        return
                    is_early = (first_pos_idx is None) or (end_idx < first_pos_idx)
                    # Determine if there are positive scoring weeks before/after
                    has_pos_before = any(
                        (pts[k] > 0 and week_nums[k] != 99) for k in range(0, start_idx)
                    )
                    has_pos_after = any(
                        (pts[k] > 0 and week_nums[k] != 99) for k in range(end_idx+1, n)
                    )
                    start_week = week_nums[start_idx]
                    # Early runs: mark as suspension only when length in {2,4,6} and there are positive games later
                    if is_early:
                        ...
                        # Late runs
                        if length >= 3:
                            # 3+ game stretch → injury
                            for j in range(start_idx, end_idx+1):
                                pw.at[idxs[j], "Injury?"] = True
                        elif length == 2:
                            # two‑game runs: classify as suspension only if there are positive
                            # games both before and after and the run starts by Week 14
                            ...
                            # should never happen due to length check
                            pass

                # Find runs of consecutive zero‑point non‑bye numeric weeks
                run_start: Optional[int] = None
                for i, (p_val, w_num, bye_flag) in enumerate(zip(pts, week_nums, byes)):
                    # break on non‑numeric or bye week
                    if w_num == 99 or bye_flag:
                        if run_start is not None:
                            assign_run(run_start, i-1)
                            run_start = None
                        continue
                    if p_val == 0:
                        ...
                        if run_start is not None:
                            ...
                            run_start = None
                if run_start is not None:
                    assign_run(run_start, n-1)
        except Exception as e:
            _log_exc(repo_root / "exports/raw/build_debug.log", "heuristic_injury_susp", e)
