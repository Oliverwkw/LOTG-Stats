...
# Sleeper uses slightly different NFL team abbreviations than nfldata/nflverse.
# We normalize to nfldata-style where needed (e.g., LAR/LAC/WAS).
_TEAM_CODE_MAP = {
    "LA": "LAR",
    "STL": "LAR",
    "SD": "LAC",
...
def _norm_team(team: Optional[str]) -> Optional[str]:
    if not team:
        return None
    t = str(team).upper()
    return _TEAM_CODE_MAP.get(t, t)

def _team_name_map(users: List[Dict[str, Any]]) -> Dict[str, str]:
    """
    Map Sleeper user_id -> *user handle* (display_name).

    You explicitly want handles (e.g., Oliverwkw), not franchise/team names.
    """
    out: Dict[str, str] = {}
    for u in users or []:
        uid = str(u.get("user_id"))
        # Sleeper uses display_name as the user handle shown in the UI.
        out[uid] = u.get("display_name") or uid
    return out
...
def build_all(repo_root: Path) -> None:
    ...
    trades_rows=[]
    pick_rows=[]
    rolling_points: Dict[str, deque] = {}  # sleeper player_id -> last 5 active game points
    ...
    for lg in tqdm(leagues, desc="Seasons"):
        ...
        while True:
            ...
            for m in matchups:
                ...
                # injuries / suspensions / byes (best-effort) + hardship (points lost to injury/suspension)
                injuries_ct = susp_ct = byes_ct = 0
                hardship_pts_lost = 0.0
                status_map: Dict[str, Tuple[Optional[bool], Optional[bool], Optional[bool]]] = {}  # pid -> (inj, susp, bye)
                played_set = played_by_week.get(week, set())
                for pid in players:
                    nfl_team = _norm_team(pid_meta.get(pid, {}).get('team'))
                    pts_pid = float(ppts.get(pid, 0.0))
                    try:
                    ...
                    # Bye only if score is 0 AND team didn't play that week (after normalizing team codes).
                    bye = None
                    if pts_pid > 0:
                        bye = False
                    ...
                    if susp: susp_ct += 1
                    if bye: byes_ct += 1
                    # Hardship = points lost due to injury or suspension (NOT bye).
                    if (inj or susp) and pts_pid == 0.0 and not bye:
                        hist = rolling_points.get(pid)
                        if hist and len(hist) > 0:
                            hardship_pts_lost += (sum(hist) / len(hist))
                    status_map[pid] = (inj, susp, bye)
                hardship = round(hardship_pts_lost, 2)
                ...
                for pid in players:
                    ...
                    inj, susp, bye = status_map.get(pid, (None, None, None))
                    nfl_team_n = _norm_team(nfl_team)
                    # Safety: if somehow bye is unknown, infer from schedule but never mark bye if points > 0
                    if bye is None:
                        played_set = played_by_week.get(week, set())
                        if pts > 0:
                            bye = False
                        elif nfl_team_n and played_set:
                            bye = (nfl_team_n not in played_set)
                    ...
                    # Update rolling last-5 for hardship baseline: only count full/healthy games (exclude bye/injury/susp).
                    if pts > 0 and not inj and not susp and not bye:
                        dq = rolling_points.get(pid)
                        if dq is None:
                            dq = deque(maxlen=5)
                            rolling_points[pid] = dq
                        dq.append(float(pts))
    ...
    for i, csvf in enumerate(sorted(out_dir.glob("*.csv")), start=1):
        title = csvf.stem[:31]
        ...
        # Turn the whole used range into an Excel "Table" (filterable)
        nrows = ws.max_row
        ncols = ws.max_column
        if nrows >= 1 and ncols >= 1:
            top_left = "A1"
            bottom_right = f"{get_column_letter(ncols)}{nrows}"
            ref = f"{top_left}:{bottom_right}"

            # Unique table name (Excel requires letters/numbers/underscore, must start with letter)
            tname = re.sub(r"[^A-Za-z0-9_]", "_", title)
            if not re.match(r"^[A-Za-z]", tname):
                tname = f"T_{tname}"
            tname = f"{tname}_{i}"
